...
router.post('/start-game', async (req, res) => {
    ...
    try {
        const { minesCount, userBet } = req.body;
...
router.post('/reveal-cell-auto', async (req, res) => {
    const { selectedTiles, minesCount, userBet, isGameStarted } = req.body;
    const gameMode = 'AUTO';
...
router.post('/get-paytable', async (req, res) => {
    ...
    if (!minesCount)
        return res.status(400).json({ error: 'Invalid minesCount.' });
    try {
...
router.post('/get-unfinished-game', async (req, res) => {
    ...
    try {
        ...
        const currentStep = activeGame.gameState.opened.length;
        ...
        res.json({
            ...
            opened: activeGame.gameState.opened || [],
            paytable: paytable,
